"use strict";
/* My Performance 2.9.6 — today-only temporal consistency.
   ...
   Temporal action semantics are authoritative at the Planner API so Calendar V5 cannot race the delayed UI rebind. */
(function(){
  const Clock=window.MyPerformanceClock,E=window.MyPerformancePlannerEngine,T=window.MyPerformanceTemporalExecution;
  if(!Clock||!E||typeof state==='undefined')return;
  const VERSION=14,basePlan=E.planDay.bind(E),baseEmpty=E.emptyWindows.bind(E);
  ...
  /* Calendar UI binds before TemporalExecutionUI. Make the Planner methods themselves authoritative so a fast click
     cannot hit the legacy work-ledger-only implementation before the requestAnimationFrame rebind occurs. */
  if(T?.recordSession)E.recordSession=function(slot,date){return T.recordSession(slot,date||Clock.today())};
  if(T?.skipSlot)E.skipSlot=function(slot,date){return T.skipSlot(slot,date||Clock.today())};
  if(T?.completeMission)E.completeMission=function(slot,date){return T.completeMission(slot,date||Clock.today())};
  ...
  window.MyPerformanceTemporalConsistency={VERSION,normalize,gonePast,temporalActionsAuthoritative:!!T};
})();
