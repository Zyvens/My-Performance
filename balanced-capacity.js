"use strict";
/* Balanced Capacity — GSA 8h is a soft capacity target; study keeps a protected daily floor and real work wins over synthetic filler. */
(function(){
  ...
  const hard=x=>!!x?.q&&(x.q.fixedTime||x.q.essential||x.q.commuteBlock||x.q.externalActivity);
  ...
  const weekday=date=>{const d=dfrom(date).getDay();return d>=1&&d<=5};
  ...
  function restoreDeferredStudy(p,date){
    ...
    for(const x of deferred){
      ...
      p.slots.push(Object.assign({},x,{start,end,q:Object.assign({},x.q,{priorityLevel:'critical',capacityStudyProtected:true}),reason:'estudo mínimo protegido antes da meta flexível de GSA'}));return wanted
    }
    return 0
  }
  function carveSyntheticGsa(p,date,need){
    ...
    for(const x of candidates){
      ...
      p.slots.push({q,originDate:date,start,end,key:`${q.id}|${date}`,reason:'reserva diária protegida para concurso',capacityStudyProtected:true});remaining-=take;added+=take
    }
    return added
  }
  function protectStudy(p,date){
    if(!weekday(date))return p;let have=studyMinutes(p);if(have>=STUDY_MIN)return p;
    ...
    const gsa=gsaMinutes(p),studyTotal=studyMinutes(p);p.capacity=p.capacity||{};p.capacity.gsa=gsa;p.capacity.study=studyTotal;p.capacity.gsaSoftTarget=GSA_SOFT_TARGET;p.capacity.studyProtectedMin=STUDY_MIN;p.capacity.balancePolicy='GSA 8h é meta flexível; estudo mínimo não pode ser expulso por filler sintético.';
