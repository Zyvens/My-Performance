...
(function(){
  ...
  function occupied(plan,w,ignoreId=''){return(plan.slots||[]).filter(s=>s.id!==ignoreId&&overlap(s,w)).sort((a,b)=>a.start-b.start)}
  function freeAt(plan,w,wantStart,len){const xs=occupied(plan,w);const starts=[];if(Number.isFinite(wantStart))starts.push(Math.max(w.start,Math.min(w.end-len,wantStart)));starts.push(w.start);for(const x of xs)starts.push(Math.max(w.start,x.end));for(const start of starts){const end=start+len;if(end>w.end)continue;if(!xs.some(x=>overlap(x,{start,end})))return[start,end]}return null}
  function replaceable(plan,w,len){return(plan.slots||[]).filter(s=>s.sideQuest&&!s.fixed&&!s.pinned&&!s.eventSlot&&!s.eventTravel&&overlap(s,w)&&dur(s)>=len).sort((a,b)=>{const ae=explicitFor(a.q,w)?1:0,be=explicitFor(b.q,w)?1:0;return ae-be||rank(b.q)-rank(a.q)})[0]||null}
  ...
  function ensureWindow(plan,w,date,used){
    ...
    const candidates=all().filter(q=>eligible(q,w,date)&&!used.has(q.id)).sort((a,b)=>score(b,w,date)-score(a,w,date));if(!candidates.length)return;
    const q=candidates[0];if((plan.slots||[]).some(s=>s.q?.id===q.id&&overlap(s,w))){used.add(q.id);return}
    const len=targetMinutes(q,w,date),p=pref(q,date),want=p?Number(p[0]):w.start;let spot=freeAt(plan,w,want,len),removed=null;
    if(!spot){removed=replaceable(plan,w,len);if(removed){plan.slots=plan.slots.filter(s=>s!==removed);spot=freeAt(plan,w,want,len)}}
    if(!spot)return;
    plan.slots.push(makeSlot(q,w,date,spot[0],spot[1],explicitFor(q,w)?'Side Quest prioritária desta Janela':'Side Quest preferencial desta Janela'));used.add(q.id)
  }
  function normalize(plan,date){if(!plan||plan.pastGone||plan.discarded)return plan;const used=new Set((plan.slots||[]).filter(s=>s.sideQuest&&s.q?.id).map(s=>s.q.id));for(const w of plan.windows||[])ensureWindow(plan,w,date,used);plan.slots=(plan.slots||[]).sort((a,b)=>Number(a.start||0)-Number(b.start||0)||Number(a.end||0)-Number(b.end||0));return plan}
  ...
  window.MyPerformanceSideQuestLaneAuthority={VERSION,normalize,eligible,score,rank};
})();
