...
(function(){
  ...
  function replaceable(plan,w,len){return(plan.slots||[]).filter(s=>s.sideQuest&&!s.fixed&&!s.pinned&&!s.eventSlot&&!s.eventTravel&&!s.laneAuthorityV13&&overlap(s,w)&&dur(s)>=len).sort((a,b)=>{const ae=explicitFor(a.q,w)?1:0,be=explicitFor(b.q,w)?1:0;return ae-be||rank(b.q)-rank(a.q)})[0]||null}
  ...
  function placeCandidate(plan,w,date,q){if((plan.slots||[]).some(s=>s.q?.id===q.id&&overlap(s,w)))return true;const len=targetMinutes(q,w,date),p=pref(q,date),want=p?Number(p[0]):w.start;let spot=freeAt(plan,w,want,len);if(!spot){const removed=replaceable(plan,w,len);if(removed){plan.slots=plan.slots.filter(s=>s!==removed);spot=freeAt(plan,w,want,len)}}if(!spot)return false;plan.slots.push(makeSlot(q,w,date,spot[0],spot[1],explicitFor(q,w)?'Side Quest prioritária desta Janela':'Side Quest preferencial desta Janela'));return true}
  ...
  window.MyPerformanceSideQuestLaneAuthority={VERSION,normalize,eligible,score,rank,placeCandidate};
})();
