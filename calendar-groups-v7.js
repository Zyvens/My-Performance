...
(function(){
  ...
  function container(){const c=D.model();c.groupEventRefs=c.groupEventRefs||{};return c}
  function store(){const c=container();if(!Array.isArray(c.groups)||!c.groups.length)c.groups=clone(DEFAULTS);for(const g of DEFAULTS)if(!c.groups.some(x=>x.id===g.id))c.groups.unshift(clone(g));return c.groups}
  function groups(){return clone(store())}
  function group(id){return store().find(g=>g.id===id)||null}
  function valid(id){return!!group(id)}
  const baseGroup=B.groupForQuest.bind(B),baseUpdateWindow=D.updateWindow.bind(D),baseAddWindow=D.addWindow.bind(D),baseAddEvent=D.addEvent.bind(D),baseUpdateEvent=D.updateEvent.bind(D),baseEvent=D.event.bind(D),baseEvents=D.events.bind(D);
  B.groups=groups;
  ...
  function rememberEventGroup(id,groupId){if(id&&valid(groupId))container().groupEventRefs[id]=groupId}
  D.addEvent=function(data={}){const desired=valid(data.groupId)?data.groupId:'Pessoal',e=baseAddEvent(Object.assign({},data,{groupId:'Pessoal'}));if(e){e.groupId=desired;rememberEventGroup(e.id,desired);saveState()}return e};
  D.updateEvent=function(id,patch={}){const desired=patch.groupId&&valid(patch.groupId)?patch.groupId:null,ok=baseUpdateEvent(id,Object.assign({},patch,desired?{groupId:'Pessoal'}:{}));const e=baseEvent(id);if(ok&&e&&desired){e.groupId=desired;rememberEventGroup(id,desired);saveState()}return ok};
  D.event=function(id){const e=baseEvent(id),g=container().groupEventRefs?.[id];if(e&&valid(g))e.groupId=g;return e};
  D.events=function(){const refs=container().groupEventRefs||{};return(baseEvents()||[]).map(e=>{if(valid(refs[e.id]))e.groupId=refs[e.id];return e})};
  ...
  function remove(id,fallback='Pessoal'){const g=group(id);if(!g||g.builtin)return false;if(!valid(fallback)||fallback===id)fallback='Pessoal';const c=container(),m=B.model();for(const w of m.windows||[]){w.groups=(w.groups||[]).map(x=>x===id?fallback:x);if(w.preferredGroup===id)w.preferredGroup=fallback}for(const e of c.events||[])if((c.groupEventRefs[e.id]||e.groupId)===id){e.groupId=fallback;c.groupEventRefs[e.id]=fallback}for(const campaign of m.campaigns||[])if(campaign.groupId===id)campaign.groupId=fallback;for(const meta of Object.values(m.missionMeta||{}))if(meta.groupId===id)meta.groupId=fallback;for(const q of state.customQuests||[])if(q.domain===id)q.domain=fallback;state.overrides=state.overrides||{};for(const q of (typeof quests==='function'?quests():[]))if(q.domain===id)state.overrides[q.id]=Object.assign({},state.overrides[q.id]||{},{domain:fallback});c.groups=c.groups.filter(x=>x.id!==id);saveState();return true}
  window.addEventListener?.('my-performance-cloud-loaded',()=>{try{store()}catch{}});
