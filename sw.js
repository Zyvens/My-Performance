...
const ASSETS=['./','./index.html','./styles.css','./planner.css','./narrator.css','./calendar-v3.css','./data.js','./routine-data.js','./app.js','./scheduler-core.js','./planner.js','./calendar-model-v3.js','./campaign-policy-v3.js','./planner-engine-v4.js','./calendar-runtime-v3.js','./cloud-sync-v2.js','./notifications.js','./pwa-update.js','./calendar-ui-v3.js','./runtime-stability-v4.js','./version.json','./manifest.webmanifest','./icon.svg'];
...
self.addEventListener('activate',event=>event.waitUntil((async()=>{
  ...
  await Promise.all(windows.map(async client=>{
    try{const u=new URL(client.url);if(u.origin!==self.location.origin)return;if(u.searchParams.get('_mpv')===BUILD)return;u.searchParams.set('_mpv',BUILD);u.searchParams.set('_mpr',Date.now().toString());await client.navigate(u.toString())}catch{}
  }))
})()));
