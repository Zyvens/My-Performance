...
(function(){
  const BUILD=String(document.documentElement.dataset.build||'1.5.18');
  let scheduled=false;
  ...
  function clearFalseOverload(h){if(h.realCritical)return;document.querySelectorAll('#view .emergency-day').forEach(x=>x.remove());document.querySelectorAll('#view .card.danger').forEach(card=>{const text=(card.textContent||'').toLowerCase();if(text.includes('conflito crítico')||text.includes('horários protegidos')||text.includes('overload'))card.remove()});document.querySelectorAll('.planner-stats .mini-stat').forEach(card=>{const label=(card.querySelector('span')?.textContent||'').trim();if(label!=='Capacidade')return;card.classList.remove('danger');setText(card.querySelector('b'),'Dentro da janela');setText(card.querySelector('small'),`${fmtMin(h.used)} ocupados de ${fmtMin(h.awake)}`)})}
