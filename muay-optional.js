"use strict";
/* Muay Thai is additive to the gym session, never a replacement. */
(function(){
  ...
  function baseGymDays(){const seed=QUEST_SEED.find(q=>q.id==='personal-gym');return Array.isArray(seed?.weekdays)?seed.weekdays:[1,2,4,6]}
  function activeDays(){ensure();return state.routineSettings.muayDays}

  function applyMuay(){
    ensure();const before=JSON.stringify(state.overrides||{}),days=activeDays(),enabled=days.length>0;
    const gymDays=[...new Set(baseGymDays().concat(days))].sort((a,b)=>a-b);
    ...
    /* Gym remains 1h30; Muay adds 1h plus 10 min each way. */
    mergeOverride('routine-muay-commute',{weekdays:days,timeStart:'09:50',timeEnd:'10:00',durationMin:10,disabled:!enabled,title:'Deslocamento para Muay Thai',description:'10 minutos reservados para ida. A academia de 1h30 continua acontecendo cedo no mesmo dia.'});
    mergeOverride('routine-muay',{weekdays:days,timeStart:'10:00',timeEnd:'11:00',durationMin:60,disabled:!enabled,title:'Muay Thai — opcional',description:'Aula opcional de 1h adicionada ao treino de academia de 1h30 do mesmo dia. Com deslocamentos, a carga física/logística total do dia fica em cerca de 2h50.'});
    mergeOverride('routine-muay-return',{weekdays:days,timeStart:'11:00',timeEnd:'11:10',durationMin:10,disabled:!enabled,title:'Volta do Muay Thai',description:'10 minutos reservados para retorno. O treino de academia continua sendo uma missão separada no mesmo dia.'});
  ...
  let lock=false;
  function reconcile(){if(lock)return;lock=true;const changed=applyMuay();if(changed)saveState();lock=false}
  reconcile();
  ...
  window.MyPerformanceMuay={activeDays,applyMuay};
})();