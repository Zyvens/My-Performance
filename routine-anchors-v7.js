"use strict";
/* Routine Anchors V7 — wake/sleep are configurable day anchors, not Events. Also migrates meal capacity into dedicated windows. */
(function(){
  ...
  function migrateWindows(){const c=D.model();if(Number(c.routineWindowSchema||0)>=7)return;c.routineWindowSchema=7;
    ...
    const sleepWindows={1:'mon-sleep',2:'tue-sleep',3:'wed-sleep',4:'thu-sleep',5:'fri-sleep',6:'sat-sleep',0:'sun-sleep'};
    for(let wd=0;wd<7;wd++){const a=get(wd),id=sleepWindows[wd],start=Math.max(0,a.sleepMin-30),end=Math.min(1560,a.sleepMin+10);upsertWindow(id,{weekday:wd,start,end,label:'Encerramento / Sono',groups:['Pessoal'],preferredGroup:'Pessoal',color:'#718096',energy:'low',allowSideQuests:true,sideQuestDedicated:true,zoneFree:false,rigidity:'preferred'})}
    saveState();E.invalidate?.();
  }
  migrateWindows();
